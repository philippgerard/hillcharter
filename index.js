    ...
    const response = await prompts([
      ...
      },
      {
        ...
        choices: [
          ...
          { title: '⚙️ 60-75% - Building and implementing', value: 67 },
...
function spreadOverlappingPoints(points) {
  // Group points by x position
  const groups = {};
  points.forEach(p => {
    const key = p.progress;
    if (!groups[key]) {
      groups[key] = [];
    }
    groups[key].push(p);
  ...
  // For each group with multiple points, spread them out VERTICALLY ONLY
  const result = [];
  Object.values(groups).forEach(group => {
    if (group.length === 1) {
      result.push(group[0]);
    } else {
      // Stack points vertically, centered around the original position
      const spacing = 55; // Vertical spacing between points
      const totalHeight = (group.length - 1) * spacing;
      const startOffset = -totalHeight / 2; // Center the stack
...
async function generateChartImage(points, title) {
  ...
  const dataPoints = spreadPoints.map(p => ({
    id: p.id,
    name: p.name,
    x: p.progress,
